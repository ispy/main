...
namespace iSpyApplication
{
    public partial class AddCameraForm : Form
    {
        ...
        private bool SelectSource()
        {
            ...
            FindCamerasForm.LastConfig.PromptSave = false;
            
            var vs = new VideoSourceForm { CameraControl = CameraControl, StartWizard = StartWizard };
            vs.ShowDialog(this);
            if (vs.DialogResult == DialogResult.OK)
            {
                ...
                if (CameraControl.VolumeControl == null && CameraControl.Camera!=null)
                {
                    ...
                    if (FindCamerasForm.LastConfig.PromptSave)
                    {
        ...
        {
            var cp = new ConfigureProcessorForm(CameraControl);
        ...
        {
            var ct = new ConfigureTimestampForm
                         {
        ...
        {
            var cms = new CameraMicSourceForm
                          {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (i > -1)
            {
                var s = CameraControl.Camobject.ptzschedule.entries.ToList().OrderBy(p => p.time).ToList();
                var si = s[i];
                var cr = new ConfigureRepeatForm {Interval = 60, Until = si.time};
