...
{
    public partial class FindCamerasForm : Form
    {
        ...
        private void PortScanner(IEnumerable<int> ports, string ipaddress, ManualResetEvent mre)
        {
            ...
            if (found)
            {
                Log.Info("Ping response from " + ipaddress);
                ...
                foreach (int iport in ports)
                {
                    try
                    {
                        ...
                        {
                            Log.Warn("Web error from " + ipaddress + ":" + iport + " " + ex.Message);
                        }
                        if (response != null)
                        {
                            Log.Warn("Web response from " + ipaddress + ":" + iport + " " +
                                                      response.StatusCode);
                    ...
                    {
                        Log.Error("Web error from " + ipaddress + ":" + iport + " " + ex.Message);
        ...
        private static bool SendRTSPReq(string addr, string login, string password)
        {
            ...
            try
            {
                ...
                var request = "DESCRIBE " + addr + " RTSP/1.0\r\n" +
                              "CSeq: 1\r\n" +
                              "User-Agent: Dummy\r\n" +
                              "Accept: */*\r\n";
                ...
                if (response > 0)
                {
                    ...
                    Log.Info("RTSP attempt: " + resp + " at " + addr);
                }
                sock.Close();
            }
            catch (Exception ex)
